...
const AllCourses = () => {
  ...
  const openPreview = (_courseId: string, title: string, teacher: string) => {
    alert(`📚 ${title}\n\n👨‍🏫 Instructor: ${teacher || 'Expert Instructor'}\n\nClick "Buy Now" to enroll!`);
  };
  ...
  // Filter courses by search query (client-side filtering)
      ...
      <main className="mt-[90px] px-[5%] py-12 max-w-[1400px] mx-auto">
        ...
        {error && (
          <div className="text-center py-16">
            ...
            <h3 className="text-xl font-semibold text-slate-800 mb-2" style={{ fontFamily: "'Poppins', sans-serif" }}>Failed to load courses</h3>
            ...
            {filteredCourses.map((course) => (
              <div key={course._id} className="bg-white rounded-xl overflow-hidden transition-all duration-300 border border-slate-200 hover:-translate-y-2 hover:shadow-xl hover:border-red-600" style={{ boxShadow: '0 2px 8px rgba(0, 0, 0, 0.04)' }}>
                <div className="relative">
                  ...
                  />
                  {course.level && (
                    <span className="absolute top-3 right-3 text-white py-1.5 px-3 rounded-full text-xs font-semibold" style={{ background: 'linear-gradient(135deg, #dc2626 0%, #991b1b 100%)', fontFamily: "'Poppins', sans-serif" }}>{course.level}</span>
                  )}
                </div>
                <div className="p-6">
                  ...
                  {course.teachers?.teacherName && (
                    <p className="text-sm text-slate-600 mb-3">👨‍🏫 {course.teachers.teacherName}</p>
                  )}
